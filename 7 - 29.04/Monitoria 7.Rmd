...
# Plotar resultados
plot(?, colMeans(medias), type = 'l', ylim = c(0.49, 0.51),
     xlab = 'Número de lançamentos', ylab = 'Média das proporções de caras',
     main = 'Convergência em Probabilidade')
```
No Hansen: Definição 6.1; Teorema 6.1; Teorema 6.2; e Definição 6.3.
...
par(mfrow = c(2, 2))
for (i in 1:4) {
  hist(?[i,], main = paste("Histograma da dimensão", i), xlab = "Média Amostral", breaks = 80)
}
```
No Hansen: Definição 6.2; Teorema 6.3; Teorema 6.9; e Teorema 6.10.
...
base <- as.data.frame(lapply(?, as.numeric))
```
...
```{r}
ef_aleatorios <- # o modelo nesse caso é "random"

```
No Hansen: Tópicos 17.4, 17.6, 17.7 e 17.8.
